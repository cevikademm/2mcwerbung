...
export const LivePreview: React.FC<LivePreviewProps> = ({ creation, isLoading, isFocused, onReset }) => {
    ...
    const handleExport = () => {
        ...
        a.href = url;
        a.download = `${creation.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_analysis.json`;
      ...
      <div className="bg-[#121214] px-4 py-3 flex items-center justify-between border-b border-zinc-800 shrink-0">
        ...
        <div className="flex items-center space-x-2 text-zinc-500">
            ...
            <span className="text-[11px] font-mono uppercase tracking-wider">
                {isLoading ? 'ANALYZING DOCUMENT...' : creation ? creation.name : 'PREVIEW MODE'}
            </span>
        </div>
                ...
                <>
                    {creation.originalImage && (
                         <button 
                            ...
                            title={showSplitView ? "Show Results Only" : "Compare with Invoice"}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 w-full">
             ...
             <div className="w-full max-w-md space-y-8">
                <div className="flex flex-col items-center">
                    ...
                    <h3 className="text-zinc-100 font-mono text-lg tracking-tight">Fatura Analiz Ediliyor</h3>
                    <p className="text-zinc-500 text-sm mt-2">OCR ve Veri İşleme...</p>
                </div>
                ...
                <div className="w-full h-1 bg-zinc-800 rounded-full overflow-hidden">
                 ...
                 <div className="border border-zinc-800 bg-black/50 rounded-lg p-4 space-y-3 font-mono text-sm">
                     <LoadingStep text="Doküman taranıyor (OCR)" active={loadingStep === 0} completed={loadingStep > 0} />
                     <LoadingStep text="Fatura no ve tarih ayrıştırılıyor" active={loadingStep === 1} completed={loadingStep > 1} />
                     <LoadingStep text="Finansal tutarlar hesaplanıyor" active={loadingStep === 2} completed={loadingStep > 2} />
                     <LoadingStep text="Sonuç kartı oluşturuluyor" active={loadingStep === 3} completed={loadingStep > 3} />
                 </div>
             </div>
          </div>
        ) : creation?.html ? (
